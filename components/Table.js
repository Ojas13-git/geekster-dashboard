      ...
      <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody>
          {data.map((item, index) => (
            ...
            >
              {item.date && <td className="px-6 py-4">{item.date}</td>}
              {item.status && <td className="px-6 py-4">{item.status}</td>}
              {item.email && <td className="px-6 py-4">{item.email}</td>}
              {item.phone && <td className="px-6 py-4">{item.phone}</td>}
              {item.services && <td className="px-6 py-4">{item.services}</td>}
              {item.scheduledDate && <td className="px-6 py-4">{item.scheduledDate}</td>}
            </tr>
          ))}
        </tbody>
      </table>
